...
def example_entry_point():
    ...
    with device.start_stream():
        ...
        while True:
            ...
            def snap_shot():
                time_now = time.time()
                cv2.imshow('snapshot' + format(time_now), origin_img())
                npndarray_reshape = origin_img().reshape((item.height, item.width))
                print(npndarray_reshape.shape)
                print('Saving image')
                png_name = f'from_Dolp_to_png_with_pil' + format(time_now) + '.png'
                png_array = PIL_Image.fromarray(npndarray_reshape)
            ...
            def img_show(name, feature):
                ...
                cv2.putText(feature, text="FPS:" + str(int(fps)), org=(20, 50), fontFace=cv2.FONT_HERSHEY_TRIPLEX,
