...
def example_entry_point():
    ...
    with device.start_stream():
        ...
        while True:
            ...
            cv2.putText(npndarray, text="FPS:" + str(int(fps)), org=(0, 50), fontFace=cv2.FONT_HERSHEY_TRIPLEX,
                        fontScale=1.0, color=(0, 0, 0))

            cv2.imshow('Mono8', npndarray)
            """
            ...
            if key == 32:
                TIME = time.time()
                cv2.imshow('snapshot' + format(TIME), npndarray)
                npndarray_reshape = npndarray.reshape((item.height, item.width))
                print('Saving image')
                png_name = f'from_Mono8_to_png_with_pil' + format(TIME) + '.png'
